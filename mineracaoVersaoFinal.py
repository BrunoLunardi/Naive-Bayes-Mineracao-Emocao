# -*- coding: utf-8 -*-
"""
Created on Mon Feb  8 08:16:48 2021
...
#nltk.download()
#10 frases de alegria e 10 de medo
...
#70% da base de dados para gerar o modelo
#160 alegria
#120 de cada tipo de emoção (desgosto, medo, raiva, surpresa e tristeza)
basetreinamento = [
...
#################################################################
#30% da base para realizar testes
#48 frases de alegria
#para as outra emoções são 36 frases
...
################## inicio stop words manual##################
#stop words -> palavras que não agregam no significado das frases
stopwords = ['a', 'agora', 'algum', 'alguma', 'aquele', 'aqueles', 'de', 'deu', 'do', 'e', 'estou', 'esta', 'esta',
             'ir', 'meu', 'muito', 'mesmo', 'no', 'nossa', 'o', 'outro', 'para', 'que', 'sem', 'talvez', 'tem', 'tendo',
             'tenha', 'teve', 'tive', 'todo', 'um', 'uma', 'umas', 'uns', 'vou']

#função para remover stop words de frases
    #texto -> é a base de dados (todas as frases)
#def removestopwords(texto):
#    frases = []
#    for(palavras, emocao) in texto:
#        #adiciona em semstop todas as palavras da variavel base que não estão em stopwords
...
################## fim stop words manual##################

stopwordsnltk = nltk.corpus.stopwords.words('portuguese')
#adiciona mais stop words ao conjunto, pois a biblioteca corpous ainda não possui todas...
stopwordsnltk.append('vou')
stopwordsnltk.append('tão')
#print(stopwordsnltk)


################## inicio stop words NLTK ##################
def removestopwords(texto):
    frases = []
    for(palavras, emocao) in texto:
        #adiciona em semstop todas as palavras da variavel base que não estão em stopwords
...
################## início radical das palavras -> stemming ##################
...
#######variável com todos os textos traadtos (sem stop words e só com o radical das palavras)
#retira stopwords da base de treinamento e deixa só o radical da palavra
frasescomstemmingtreinamento = aplicastemmer(basetreinamento)
#retira stopwords da base de teste e deixa só o radical da palavra
frasescomstemmingteste = aplicastemmer(baseteste)
#print(frasescomstemmingtreinamento)
#######variável com todos os textos tratos (sem stop words e só com o radical das palavras)


################## início listar as palavras do texto ##################
...
#retorna todas as palavras presente na base de dados treinamento
palavrastreinamento = buscapalavras(frasescomstemmingtreinamento)
#retorna todas as palavras presente na base de dados teste
palavrasteste = buscapalavras(frasescomstemmingteste)

# print(palavras)
# print()

################## fim listar as palavras do texto ##################

################## início palavras únicas (para gerar tabela) ##################
def buscafrequencia(palavras):
    #conta a frequencia de repetição das palavras
    palavras = nltk.FreqDist(palavras)
    return palavras

#envia como parâmetro  os radicais das palavras, sem as emoções associadas
...
def buscapalavrasunicas(frequencia):
    #separa palavras sem repetições
...
#854 palavras (854 colunas na tabela de Naive Bayes)
#print(palavrasunicastreinamento)
# print()
################## fim palavras únicas (para gerar tabela) ##################

################## início verifica se tem a palavra na frase (gerar valores da tabela) ##################

def extratorpalavras(documento):
    doc = set(documento)
    caracteristicas = {}
    
    #percorre todas as palavras da variável palavrasunicas (resultante da func buscapalavrasunicas)
    for palavras in palavrasunicastreinamento: 
        #para cada texto(representado por doc), verifica se existe cada uma das palavras da lista palavrasunicas
...
################## fim verifica se tem a palavra na frase (gerar valores da tabela)) ##################

################## início verifica se tem a palavra em todo o texto (gerar valores da tabela)) ##################

#parametros:
    #a função de verificação de palavras (verifica se existe a palavra no texto)
    #a variável com a base de dados tratada (todo texto sem stop words e com radicais)
...
#cada linha tem todas as palavras únicas, e cada linha apresenta o valor True ou False para cada palavra.
    #se for True, indica que para a frase correspondente aquela linha, existe a palavra
    #se for False, não existe a palavra para aquela frase
    #assim preenche os valores da tabela de característica das frases
# print(basecompleta)
# print()

################## fim verifica se tem a palavra em todo o texto (gerar valores da tabela)) ##################
...
#testa a precisão do algoritmo com a base de treinamento
    #logo dará quase 100%, por que foi utilizada a mesma base de dados para gerar o modelo (tabela Naive Bayes)
# print(nltk.classify.accuracy(classificador,basecompletatreinamento))
#testa precisão do algoritmo com os dados de teste
#print(nltk.classify.accuracy(classificador,basecompletateste))

################### início verificando erros do algortimo ###################
erros = []
...
for (frase,classe) in basecompletateste:
    ...
    #verifica cada frase da basecompletateste
    #resultado -> é a previsão do algoritmo em relação a cada frase da basecompleteste
    resultado = classificador.classify(frase)
    
    if resultado != classe:
        #append a classe que era pra ser a correta, o resultado classificado pelo algortimo e a frase correspondete
        erros.append((classe, resultado, frase))

#exibe toodos os dados que foram classificados errados
# for(classe, resultado, frase) in erros:
#     print(classe, resultado, frase)

################### fim verificando erros do algortimo ###################

#rótulos targets/labels/classes
#print(classificador.labels())
#print("######################")
#atributos mais importantes da base de texto (atributos mais informativos)
    #(n) -> número de palavras que deseja ver
#print(classificador.show_most_informative_features(20))

#teste de uma frase a ser classificada pelo Naive Bayes
...
#tratando a variável teste (deixando somente os radicais)
testestemming = []
...
#envia o texto todo para a classificação (junto com a base de dados)
    #valores da tabela True e False do Naive Bayes
    #True para os radicais que existem na frase informada no testestemming
novo = extratorpalavras(testestemming)
# print(novo)

#classifica com Naive Bayes
# print(classificador.classify(novo))

#retornar a probablidade para cada uma das classes, em relação ao texto informado
...
################## início matriz de confusão ##################
from nltk.metrics import ConfusionMatrix
################### teste didático ###################
#dados de treino (dados inventados só para testar a matriz(didático))
#esperado = 'alegria alegria alegria alegria medo medo surpresa surpresa'.split()
#resultado que o classificador fez a previsão (teste) (dados inventados só para testar a matriz(didático))
...
for (frase, classe) in basecompletateste:
    #pega a base de dados de teste e passa cada frase dele para o classificador
        #cada frase já vem com o padrão da tabela Naive Bayes, ou seja,
        #vem com todas as palavras únicas da base inteira de dados.
        #se a palavra da frase tiver na base de dados de palavras únicas, então o valor é True, senão é False
        #classifica a frase de acordo com estes dados
    resultado = classificador.classify(frase)
    #como esta base de dados já veio "classificado", ou seja, cada frase já tem um label definido, então
    #utiliza estes labels como resultado previsto. Logo a matriz irá comparar os dados do labels com o que
    #foi classificado
    
    #previsto -> resultado da classificação do modelo gerado
    previsto.append(resultado)
    #esperado -> label que ja estava na base de dados (frase, classe)
